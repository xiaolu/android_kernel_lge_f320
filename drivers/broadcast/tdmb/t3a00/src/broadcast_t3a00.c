...
static int power_set_pm8941_l10(int onoff);
...
static int power_set_pm8941_l10(int onoff)
{
	...
	if((lge_get_board_revno() == HW_REV_A)){
		vreg_l10 = NULL;
		printk("[T3A00]VU3 RevA does not support PMIC L10 1.8V\n");
		return rc;
	}
#endif
	...
	{
		vreg_l10 = NULL;
		printk("[T3A00]Z-KOR RevA or RevB does not support PMIC L10 1.8V\n");
		return rc;
	}
#endif
	...
	if(!vreg_l10 ) {
		vreg_l10 = regulator_get(&t3a00_ctrl_info.spi_ptr->dev, "tdmb-vdd-io");
		if(IS_ERR(vreg_l10)) {
			printk("%s: regulator get of pm8941_l10 failed in T3A00 (%ld)\n",
	...
	if(onoff){
		rc = regulator_set_voltage(vreg_l10 , 1800000, 1800000);
		if(rc < 0 ) {
			printk("[T3A00] %s: cannot control regulator pm8941 l10 1.8V :%d\n", __func__, rc);
			goto vreg_l10_fail;
		}
	...
	printk("[T3A00]pm8941 l10 voltage contorl success mode =(%d)\n", onoff);
	return rc;
