...
*/
	...
	if (use_pm8941_xo_a2_192000 == 1) {
		...
		clk = clk_get(&spi->dev, "xo");
		if ( IS_ERR(clk) )
		{
			ret = PTR_ERR(clk);
			PRINTF(0,"[1seg] fc8150_spi_probe clk_get error!!!\n");
			use_pm8941_xo_a2_192000 = 0; // disable pm8941_xo_a2
			return ret;
		}	
		/* We enable/disable the clock only to assure it works */
		ret = clk_prepare_enable(clk);
		if (ret) {
			PRINTF(0,"[1seg] fc8150_spi_probe clk_prepare_enable error!!!\n");			
			return ret;
		}
