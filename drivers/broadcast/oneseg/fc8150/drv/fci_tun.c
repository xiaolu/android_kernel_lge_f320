...
int tuner_ctrl_select(HANDLE hDevice, i2c_type type)
{
	switch (type) {
		...
		case FCI_BYPASS_TYPE:
			...
			break;
		default:
			PRINTF(hDevice, "[1seg] FC8150 tuner_ctrl_select BBM_E_TN_CTRL_SELECT : %d \n", BBM_E_TN_CTRL_SELECT);
			return BBM_E_TN_CTRL_SELECT;
	}

	if(tuner_ctrl->init(hDevice, 600, 0))
	{
		PRINTF(hDevice, "[1seg] FC8150 tuner_ctrl_select BBM_E_TN_CTRL_INIT : %d \n", BBM_E_TN_CTRL_INIT);
