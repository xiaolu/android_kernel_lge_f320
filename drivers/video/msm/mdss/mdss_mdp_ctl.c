...
extern u32 mdss_display_id(struct mdss_panel_data *ctrl);
extern u32 display_id;
extern void mdss_dsi_cmd_bta_sw_trigger(struct mdss_panel_data *pdata);
#endif
void mdss_mdp_ctl_splash_start(struct mdss_panel_data *pdata)
{
	...
	case MIPI_CMD_PANEL:
#if defined(CONFIG_MACH_LGE) && !defined(CONFIG_MACH_MSM8974_VU3_KR) && !defined(CONFIG_MACH_MSM8974_Z_KR) && !defined(CONFIG_MACH_MSM8974_Z_US) && !defined(CONFIG_OLED_SUPPORT)
	if((lge_get_boot_mode()==LGE_BOOT_MODE_FACTORY2)||(lge_get_boot_mode()==LGE_BOOT_MODE_PIFBOOT)||(lge_get_boot_mode()==LGE_BOOT_MODE_PIFBOOT2) ||(lge_get_boot_mode()==LGE_BOOT_MODE_MINIOS)){
		mdss_dsi_cmd_bta_sw_trigger(pdata);
		mdss_display_id(pdata);
		pdata->panel_info.panel_id = display_id;
		pr_info("panel_id = %d\n", display_id);
	}
#endif
	default:
