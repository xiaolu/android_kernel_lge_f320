...
static char id_cmd[2] = {0x04, 0x00}; /* DTYPE_DCS_READ */
static struct dsi_cmd_desc id_read_cmd = {
	{DTYPE_DCS_READ, 1, 0, 1, 5, sizeof(id_cmd)},
	id_cmd
};
u32 display_id = 0;
static void id_read_cb(u32 data)
{
	pr_info("%s: data=0x%x\n", __func__, ntohl(data));
	display_id = data;

}
u32 mdss_display_id(struct mdss_panel_data *pdata)
{
	struct dcs_cmd_req cmdreq;
struct mdss_dsi_ctrl_pdata *ctrl = NULL;
	...
	memset(&cmdreq, 0, sizeof(cmdreq));
	cmdreq.cmds = &id_read_cmd;
	cmdreq.cmds_cnt = 1;
	cmdreq.flags = CMD_REQ_RX | CMD_REQ_COMMIT;
	cmdreq.rlen = 1;
	cmdreq.cb = id_read_cb; /* call back */
	mdss_dsi_cmdlist_put(ctrl, &cmdreq);
	/*
	 * blocked here, until call back called
	 */
...
#ifdef CONFIG_MACH_LGE
	panel_data->panel_info.panel_id = 1;
#endif
