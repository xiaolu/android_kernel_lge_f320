...
#define LAST_UPDATE  "13-07-01, 0x0204N_1"
...
/* 8bit */
OISINI__ const struct STHALREG	CsHalReg[][32]	= {
...
/* 16bit */
OISINI__ const struct STHALFIL	CsHalFil[][138]	= {
	{	//VER 04 /*FC filename : Mitsumi_130501*/
...
/* 32bit */
OISINI__ const struct STGYRFIL	CsGyrFil[][125]	= {
	{	//VER 04
...
void	IniGyr( void )
{
	...
	//RamWrite32A( gxlmt3H0, 0x3EBD70A4 ) ;	// 0x18B4
	//RamWrite32A( gylmt3H0, 0x3EBD70A4 ) ;	// 0x19B4	Limiter = 0.37, 1.0deg
	//RamWrite32A( gxlmt3H1, 0x3EBD70A4 ) ;	// 0x18B5
	//RamWrite32A( gylmt3H1, 0x3EBD70A4 ) ;	// 0x19B5
...
void	VSModeConvert( unsigned char UcVstmod )
{
	...
	if( UcVstmod ) {								// Degree Mode Convert
		RegWriteA( GPANADDA, 0x14 ) ;				// 0x0130	GXH1Z2/GYH1Z2
		RegWriteA( GPANADDB, 0x0E ) ;				// 0x0131	GXI3Z/GYI3Z
		RegWriteA( GPANSTT31JUG0, 	0x01 );			// 0x0142
		RegWriteA( GPANSTT31JUG1, 	0x00 );			// 0x0143
		...
		RegWriteA( GPANADDA, 0x0E ) ;				// 0x0130	GXI3Z/GYI3Z
		RegWriteA( GPANADDB, 0x14 ) ;				// 0x0131	GXH1Z2/GYH1Z2
		RegWriteA( GPANSTT31JUG0, 	0x07 );			// 0x0142
		RegWriteA( GPANSTT31JUG1, 	0x00 );			// 0x0143
		RamWrite32A( Sttx12aM, 	GYRA12_MID_DPS );	// 0x184F
		RamWrite32A( Sttx12aH, 	GYRA12_HGH_DPS );	// 0x185F
		RamWrite32A( Sttx34aM, 	GYRA34_MID_DPS );	// 0x188F
		RamWrite32A( Sttx34aH, 	GYRA34_HGH_DPS );	// 0x189F
		RamWrite32A( Stty12aM, 	GYRA12_MID_DPS );	// 0x194F
		RamWrite32A( Stty12aH, 	GYRA12_HGH_DPS );	// 0x195F
		RamWrite32A( Stty34aM, 	GYRA34_MID_DPS );	// 0x198F
		RamWrite32A( Stty34aH, 	GYRA34_HGH_DPS );	// 0x199F
	}
...
int32_t	lgit_ois_off( void )
{
	...
	for (i=0x2C; i>=0;  i-=2)
	{
		RamWriteA( LXOLMT,	i << 8 );	
		RamWriteA( LYOLMT,	i << 8 );
		usleep(5000);
	}
	SrvCon( X_DIR, OFF ) ;      
    SrvCon( Y_DIR, OFF ) ;
...
int32_t lgit_ois_mode(enum ois_mode_t data)
{
	...
	switch(data)
	{
		...
		case OIS_MODE_VIDEO :
			S2cPro( OFF ) ;	//<-goto Video mode
			break;
		case OIS_MODE_CAPTURE :		
			S2cPro( ON ) ;	//<-goto Capture mode
			break;
		case OIS_MODE_CENTERING_ONLY :
			OisOff();
			break;
...
uint16_t lgit_convert_float32(uint32_t float32, uint8_t len)
{
	uint8_t sgn = float32 >> 31;
	uint16_t exp = 0xFF & (float32 >> 23);
	uint32_t frc = (0x7FFFFF & (float32)) | 0x800000;
	uint16_t out = 0;
	//printk("%s float32 %x, sqn %x exp %x frac %x ",__func__, float32, sgn, exp, frc);
...
int32_t lgit_ois_stat(struct msm_sensor_ois_info_t *ois_stat)
{
	...
	ois_stat->gyro[0] =  lgit_convert_float32(gyro,18)*256/GYRO_SCALE_FACTOR;
	CDBG("%s gyrox %x, %x \n",__func__, gyro, ois_stat->gyro[0]);
		
	RamReadA(LXGZF, &hall);
	ois_stat->target[0] = -1*(hall*256/HALL_SCALE_FACTOR);
	CDBG("%s targetx %x, %x \n",__func__, gyro, ois_stat->target[0]);

	RamReadA(HXIN, &hall);
	ois_stat->hall[0] = (hall)*256/HALL_SCALE_FACTOR;
	CDBG("%s hallx %x, %x \n",__func__, hall, ois_stat->hall[0]);
	
	RamRead32A(GYADZ, &gyro);
	ois_stat->gyro[1] =  lgit_convert_float32(gyro,18)*256/GYRO_SCALE_FACTOR;
	CDBG("%s gyroy %x, %x \n",__func__, gyro, ois_stat->gyro[1]);

	RamReadA(LYGZF, &hall);
	ois_stat->target[1] = -1*(hall*256/HALL_SCALE_FACTOR);
	CDBG("%s targety %x, %x \n",__func__, gyro, ois_stat->target[1]);
	
	RamReadA(HYIN, &hall);
	ois_stat->hall[1] = (hall)*256/HALL_SCALE_FACTOR;
	...
	RamRead32A(GXI3Z, &gyro);
	ois_stat->target[0] = lgit_convert_float32(gyro,18)*256/GYRO_SCALE_FACTOR;
	CDBG("%s targetx %x, %x \n",__func__, gyro, ois_stat->target[0]);
	...
	RamRead32A(GYI3Z, &gyro);
	ois_stat->target[1] = lgit_convert_float32(gyro,18)*256/GYRO_SCALE_FACTOR;
	CDBG("%s targety %x, %x \n",__func__, gyro, ois_stat->target[1]);
