...
static void charging_information(struct work_struct *work)
{
	...
	u8 fet_irq=0;		u8 bat_force=0;	u8 bat_fet=0;
	int bat_temp=0;
	...
	rc = qpnp_chg_read(qpnp_chg, &fet_irq, INT_RT_STS(qpnp_chg->bat_if_base), 1);
	fet_irq &= BAT_FET_ON_IRQ;
	rc = qpnp_chg_read(qpnp_chg, &bat_force, qpnp_chg->chgr_base + CHGR_CHG_CTRL, 1);
	bat_force &= CHGR_ON_BAT_FORCE_BIT;
	rc = qpnp_chg_read(qpnp_chg, &bat_fet, qpnp_chg->bat_if_base + SBMM_BAT_FET, 1);
	bat_fet &= BAT_FET_ON;
	...
	0x%x , 0x%x , 0x%x , %d , %d, 0x%x , %d(14) , %d , %d\n",
		usb_in,dc_in,vin,vbat,vdet,maxim_volt,maxim_soc,iusb,ibatm,ibat_term,
		ibat,fet_irq,bat_force,bat_fet,bat_temp,(int)xo_therm.physical,
