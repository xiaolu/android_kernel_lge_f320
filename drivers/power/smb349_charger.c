...
exception_handling:
	...
	if(smb349_chg->usb_present == true && !smb349_is_charger_present(client)){
		smb349_chg->usb_present = false;
		power_supply_set_present(smb349_chg->usb_psy,false);
		pr_err("exception_handling : set usb present as false\n");
	}
 ...
 */
static void vzw_fast_chg_change_state(struct smb349_struct *smb349_chg)
{
	u8 val;
	int ret;
...
static void vzw_fast_chg_detect_aicl_result(struct smb349_struct *smb349_chg)
{
	u8 val;
	int ret;
	...
	if (val & AICL_COMPLETE_STATUS_BIT)
		...
		vzw_fast_chg_change_state(smb349_chg);
}
#endif
...
{
	int ret, count = 0, threshold= 4400000;
...
static void smb349_irq_worker(struct work_struct *work)
{
	 ...
	 */
	ret = smb349_read_block_reg(smb349_chg->client, IRQ_A_REG, 6, irqstat);
	if (ret) {
		pr_err("Failed to read IRQ status block = %d\n", ret);
		return;
	}

	pr_info("[IRQ] A:0x%02X, B:0x%02X, C:0x%02X, D:0x%02X, E:0x%02X, F:0x%02X\n",
		irqstat[0],irqstat[1], irqstat[2], irqstat[3], irqstat[4], irqstat[5]);

	/* 3Ah - IRQ status F - irqstat[5] */
	if ((irqstat[5] & OTG_OC_LIMIT_MASK) || (irqstat[5] & OTG_BATT_UV_MASK)) {
		if (irqstat[5] & OTG_OC_LIMIT_MASK) {
		...
		} else if (irqstat[5] & OTG_BATT_UV_MASK) {
...
#ifdef CONFIG_SMB349_VZW_FAST_CHG
	vzw_fast_chg_detect_aicl_result(smb349_chg);
#endif

	/* 38h - IRQ status D - irqstat[3] */
	/* Timeout handling */
	if ( (irqstat[3] & (BIT(0) | BIT(2))) ) {
...
#if SMB349_BOOSTBACK_WORKAROUND
	if (!is_factory_cable() || is_factory_cable_130k())
#else
	if (!is_factory_cable())
#endif
	{
...
#ifdef CONFIG_SMB349_VZW_FAST_CHG
static void vzw_fast_chg_stop_charging(struct smb349_struct *smb349_chg)
{
	...
	if (is_factory_cable() && smb349_is_charger_present(smb349_chg->client)) {
		...
		if ((ret.intval / 1000) <= SMB349_CHG_IUSB_MAX_MIN_MA)
			...
			smb349_chg_usb_suspend_enable(smb349_chg, 0);
				...
				smb349_is_charger_present(smb349_chg->client)) {
#ifdef CONFIG_SMB349_VZW_FAST_CHG
		vzw_fast_chg_stop_charging(smb349_chg);
#else
		smb349_usb_hc_mode(smb349_chg, USB_HC_MODE_BIT);
		/* ToDo : Must implements & verify. */
#endif
...
static void smb349_status_print(struct smb349_struct *smb349_chg)
{
	...
	/* 38h */
	rc = smb349_read_reg(client, IRQ_D_REG, &val_38);
	if (rc) {
	...
	/* 35h */
	rc = smb349_read_reg(client, IRQ_A_REG, &val_35);
	if (rc) {
		pr_err("Failed to read IRQ_A_REG rc=%d\n", rc);
		return;
	}
	/* 36h */
	rc = smb349_read_reg(client, IRQ_B_REG, &val_36);
	if (rc) {
		pr_err("Failed to read IRQ_B_REG rc=%d\n", rc);
		return;
	}
	/* 37h */
	rc = smb349_read_reg(client, IRQ_C_REG, &val_37);
	if (rc) {
