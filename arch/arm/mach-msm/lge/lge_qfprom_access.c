...
#define SERIAL_NUMBER			0xFC4B81F0 
...
static ssize_t qfprom_read_serial_number(struct device *dev, struct device_attribute *attr, char *buf)
{
	u32 *p_buf = NULL;
	int ret = 0;
	u32 qfprom_serial_number=0;
	...
	msleep(10);
	ret = qfuse_read_single_row(SERIAL_NUMBER, 0, p_buf);
	qfprom_address = 0;
	if(ret == 0){
		qfprom_serial_number = p_buf[0];
	}
	else if(ret < 0)
		printk("%s: scm call fail\n", __func__);
	else
		printk("%s: qfprom serial number read error = %x\n", __func__, ret);

	kfree(p_buf);

	return sprintf(buf, "%x\n", qfprom_serial_number);
}
static DEVICE_ATTR(serial_number, S_IWUSR | S_IRUGO, qfprom_read_serial_number, NULL);
